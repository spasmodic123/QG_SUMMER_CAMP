...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        'input_size是embed_size+hidden_size的原因是,根据文档,decoder的输入是目标词向量和上一个和时间步的输出向量的concatenate'
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        '通过索引获取词向量,X是三维向量,第一个维度是几个句子,第二个维度是每个句子有几个单词,第三个维度是每个单词的有几个维度'
        X = self.model_embeddings.source(source_padded)

        '将多个句子打包成输入LSTM的格式,(seq_length, batch_size, input_size)'
        X = pack_padded_sequence(X, source_lengths)

        'LSTM会返回output,(最后的hidden_state,最后的cell_state)'
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X)
        enc_hiddens, len_pad = pad_packed_sequence(enc_hiddens, batch_first=True)

        '将前向和反向的encoder结果concatenate在一起在使用Wh进行映射'
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        enc_hidden_project = self.att_projection(enc_hiddens)  # shape (b, src_len, h)

        '获取输入向量'
        Y = self.model_embeddings.target(target_padded)  # shape shape (tgt_len, b, e)

        '一个batch有多个句子,对每一个句子计算attention'
        for i in torch.split(Y, 1, dim=0):
            Y_t = i.squeeze()
            '将输入向量和上一个时间步的输出向量结合'
            Y_t_bar = torch.cat((Y_t, o_prev), dim=1)  # Y_t.shape (b, e), o_prev.shape (b, h)
            dec_state ,o_t, e_t = self.step(Y_t_bar, dec_state, enc_hiddens, enc_hidden_project, enc_masks)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        ###     Tensor Squeeze:
        ###         https://pytorch.org/docs/stable/torch.html#torch.squeeze
        (decoder_hidden, decoder_cell) = self.decoder(Ybar_t, dec_state)
        dec_state = (decoder_hidden, decoder_cell)
        e_t = torch.bmm(enc_hiddens_proj, decoder_hidden.unsqueeze(2)).squeeze(2)
        ...
        '将attention和decoder的hidden_state拼在一起'
        u_t = torch.cat((a_t, decoder_hidden), dim=1)
