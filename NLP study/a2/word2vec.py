...
def naiveSoftmaxLossAndGradient(
    ...
    '(N个上下文单词,向量长度) * (1,向量长度)  因为是矩阵和向量相乘,centerWordVec会自动转化为列向量形式(向量长度,1),但本质还是一维的数组,结果是(N,1),但本质还是一维数组(1,N)'
    ...
    y = np.zeros_like(y_hat)  # (N,1)
    y[outsideWordIdx] = 1
    # can also get loss as -np.dot(y, np.log(y_hat))

    gradCenterVec = np.dot(y_hat - y, outsideVectors)  # (1,N) * (N,向量长度) = (1,向量长度)
    # or gradCenterVec = np.dot(outsideVectors.T, y_hat - y)

    '外积'
    gradOutsideVecs = np.outer(y_hat - y, centerWordVec)
    # outer product results in a matrix  (1,N) * (1,向量长度) = (N,向量长度),对应的预测概率减去真实概率再乘以中心词向量,每一行就是对应一个上下文词向量的梯度
...
def negSamplingLossAndGradient(
    ...
    'Calculate the second term,负采样部分的梯度'
    for i in range(K):
        w_k = indices[i + 1]
...
def word2vec_sgd_wrapper(word2vecModel, word2Ind, wordVectors, dataset, windowSize, word2vecLossAndGradient):
    batchsize = 50
    loss = 0.0
