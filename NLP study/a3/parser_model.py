...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        @param embeddings (ndarray): word embeddings (num_words, embedding_size),'这里的embedding_size是不词向量本身的长度,还包括了词性和依赖关系的长度!!!'
        @param n_features (int): number of input features
        @param hidden_size (int): number of hidden units
        @param n_classes (int): number of output classes
        @param dropout_prob (float): dropout probability
        """
        '神经依存分析模型的输入是一个状态的输入,包括了词向量,词性,词依赖关系'
        ...
        # print(embeddings.shape)  # (100, 30),100个单词,但不是每个单词的词向量本身长30,而是加上词性和依赖关系长30
        ...
        '''输入到隐藏层的权重矩阵,self.n_features是特征,也就是s1(stack单词1),s2(stcak单词2),b1(buffer单词1),lc(s1)表是s1左侧的依赖词这些东西,也即是行数
        embed_size就是一行过去,加上word,加上pos,加上dep一共有多长'''
    ...
    '此功能将所有词向量按照batch_size选择一部分,再堆叠起来转化成tensor形式,形成最终模型的输入格式'
    def embedding_lookup(self, w):
        ...
        'index_select函数,从embeddings中选择,选择的维度是0,索引是w[b],w.shape[0]是batch_size,batch_size多少,就拿多少个状态的向量'
        ...
        '因为要把每一个状态展平,所以列是self.n_features * self.embed_size,   每一行就是一个状态(batch_size的其中一个)'
